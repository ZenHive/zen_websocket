...
- **Error scenario test coverage** — Added explicit coverage for previously-untested error paths (R011):
  ...
  - **Deferred:** Client-level pending-request cleanup across reconnect is covered at the correlator layer but not end-to-end via `client_test.exs`. Inspection of `client.ex` showed `pending_requests` is initialized empty at construction but is never explicitly cleared or failed-back to callers on Gun disconnect — writing that integration test would risk discovering unrelated behavior; captured here for a future task.
